...
identity contract, adds Hoa-Don, inventory, and Gmail projection boundaries,
enforces an immutable write budget, and consumes a dedicated one-shot marker
before the first lease or external write.

The only permitted success path is `VALIDATED -> FILES_SAVED -> COMMITTING ->
ROWS_COMMITTED -> INVENTORY_PENDING -> PROJECTIONS_COMMITTED -> COMPLETED`.
Unknown mutation outcomes enter `PENDING_LATE_COMPLETION_QUARANTINE` without
speculative retry or lease finalization. The runtime has no create-job,
successor-attempt, attachment-record, or reconciliation-report path.

The initial binding at revisions `261`/`262` completed and released at
