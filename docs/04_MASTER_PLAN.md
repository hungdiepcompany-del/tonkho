...
D2 Firestore adapter is locally implemented over injected fake/emulator-compatible transport. Next allowed phase is D4 durable scanner integration design only. Do not wire scanners, run production Firestore access, push GAS, deploy Firebase, mutate production data, or run repair without a separate owner marker.
