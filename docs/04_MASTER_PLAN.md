...
D4 durable scanner integration design is complete. Next allowed phase is D5A local durable orchestration implementation only. Do not wire production scanners, run production Firestore access, push GAS, deploy Firebase, mutate production data, or run repair without a separate owner marker.
