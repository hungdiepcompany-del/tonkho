...
D3 report-only reconciliation is locally implemented. Next allowed phase is a separately approved D2 Firestore adapter local slice. Do not wire scanners, run production reconciliation, push GAS, deploy Firebase, mutate production data, or run repair without a separate owner marker.
