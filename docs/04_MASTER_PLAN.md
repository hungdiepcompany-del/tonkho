...
D5B local shadow runner is complete. Next allowed phase is D5C production read-only snapshot adapters only. Do not wire production scanners, run production Firestore writes, push GAS, deploy Firebase, mutate production data, or run repair without a separate owner marker.
